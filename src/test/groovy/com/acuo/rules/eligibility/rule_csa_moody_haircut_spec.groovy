...
class rule_csa_moody_haircut_spec  extends Specification {
    ...
    def "a bond has haircut 0.01 in CSA moody rules with ccy USD"() {
        when: "add a bond asset"
        def asset = new LocalAsset(currency: "USD", id: "csa_fitch_govt",type: "bond",maturityYears: 0.5, fitchRating:"F1+", rateType: "fix")
    ...
    def "a bond has haircut 0.02 in CSA moody rules with government agency"() {
        when: "add a bond asset"
        def asset = new LocalAsset(currency: "USD", id: "csa_fitch_govt",type: "bond",maturityYears: 0.5, rateType: "fix")
        def issuer = new Issuer(name: "Department of Transportation")
        def eligible = new Eligible(classType: "")
        def agreement = new LocalAgreement(trigger: 2)
        ksession.insert(asset)
        ksession.insert(issuer)
        ksession.insert(eligible)
        ksession.insert(agreement)
        ...
        then: "then we get rules regime and class"
        eligible.haircut == 0.06
    }
    def "a bond has haircut 0.01 in CSA moody rules with moody rating"() {
        when: "add a bond asset"
        def asset = new LocalAsset(datascopeAssetType: "GOVT", currency: "EUR", id: "csa_fitch_govt",type: "bond",maturityYears: 0.5, rateType: "fix", moodyRating: "Aa1")
        def issuer = new Issuer()
        def eligible = new Eligible(classType: "")
        def agreement = new LocalAgreement(trigger: 1)
        ksession.insert(asset)
    ...
    def "a bond has haircut 0 in CSA moody rules with UK"() {
        when: "add a bond asset"
        def asset = new LocalAsset(datascopeAssetType: "GOVT", currency: "GBP", id: "csa_fitch_govt",type: "bond",maturityYears: 0.5, rateType: "fix")
